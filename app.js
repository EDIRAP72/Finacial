...
// ===============================
// ORDER BOOK (SIMULATO SU PREZZO FINNHUB)
// ===============================
...
// ===============================
// DETTAGLIO: GRAFICO + TUTTO IL RESTO
// ===============================
async function createChart(symbol, realData, projected) {
...
async function showDetailPage(tickerSymbol) {
    ...
    detailPage.innerHTML = `
        ...
        <div class="order-section">
            ...
            <p><strong>Prezzo attuale:</strong> <span id="live-price">${realQuote ? realQuote.price.toFixed(2) : "N/D"}</span></p>
            <div style="display:flex; gap:40px;">
                <div>
                    <h4>Bid</h4>
                    ${orderBook ? orderBook.bids.map(b => `
                        <div>${b.price} — Vol: ${b.volume}</div>
                    `).join("") : "N/D"}
                </div>
                <div>
                    <h4>Ask</h4>
                    ${orderBook ? orderBook.asks.map(a => `
    ...
    document.getElementById("add-portfolio-btn").addEventListener("click", () => {
        ...
        const price = parseFloat(document.getElementById("live-price").innerHTML);
    ...
    const projectedData = await createChart(tickerSymbol, realData, projected);
...
function init() {
    ...
    console.log("App inizializzata — versione completa Step 1–13 consolidata");
}
